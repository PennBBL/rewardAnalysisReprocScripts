...
export SUBJECTS_DIR=/data/joy/BBL/studies/reward/processedData/structural/freesurfer53
export OUTPUT_DIR=/data/joy/BBL/projects/rewardAnalysisReproc/qa/output
...
slist=/data/joy/BBL/projects/rewardAnalysisReproc/n477_scanid_bblid_date_datexscanid.csv
#outdir=/data/joy/BBL/projects/rewardAnalysisReproc/qa/output
outdir=$OUTPUT_DIR


file=$outdir/cnr_reward.csv
euler_file=$outdir/euler_reward.csv
...
for i in $(cat $slist);do
	...
	scanid=`echo "$i" | awk -F ',' '{print $4}'`
	echo "working on subject" $bblid/$scanid
	surf=`ls -d $SUBJECTS_DIR/$bblid/$scanid/surf`
	mri=`ls -d $SUBJECTS_DIR/$bblid/$scanid/mri`
		###############CNR#######################		
		#calculate cnr for each subject and output all measures to a file in their stats folder
		mri_cnr $surf $mri/orig.mgz > $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_cnr.txt"
		...
		total=`grep "total CNR" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_cnr.txt"`
		total2=`echo $total |cut -f 4 -d " "`
		cnr=`grep "gray/white CNR" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_cnr.txt"`
		...
		script -c "mris_euler_number $surf/lh.orig.nofix" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_lh_euler.txt"
		script -c "mris_euler_number $surf/rh.orig.nofix" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_rh_euler.txt"
		#create variables for left and right euler numbers
		left=`grep ">" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_lh_euler.txt" | cut -d ">" -f 1 | cut -d "=" -f 4 | cut -d " " -f 2`
		right=`grep ">" $SUBJECTS_DIR/$bblid/$scanid/stats/$bblid"_"$scanid"_rh_euler.txt" | cut -d ">" -f 1 | cut -d "=" -f 4 | cut -d " " -f 2`
		#append this subject's data to the aggregate file
