...
groupout=/data/joy/BBL/studies/reward/summaryScores/clinical/subjectLists/all_cohort_T1Paths_053117.csv
outname1=*.nii.gz

#cleanup logs
rm -f $groupout	
rm -f $logdir/t1_nifti_missing_053117.txt
...
for s in $subj1; do 
	...
	#Save full T1 path
	echo "$bblid,$dos,$t1_nifti" >> $groupout
...
for s in $subj1; do 
	...
	#Save full T1 path
	echo "$bblid,$dos,$t1_multiecho_nifti" >> $groupout
