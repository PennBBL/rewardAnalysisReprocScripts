...
library("RCurl")
source("/data/joy/BBL/projects/rewardAnalysisOld/rewardAnalysisScripts/summaryScores/redcap_read_rdh.R")
...
#V4 by AGG: Filling in blank procedures

for (i in which(project_data$procedure == "")) {
  project_data$procedure[i] <- strsplit(split="_", project_data$participant_id[i])[[1]][2]
}
...
output$version<- do.call(paste, c(as.list(as.character(version)), sep=","))
